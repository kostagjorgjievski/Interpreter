...
class Interpreter(InterpreterBase):
    ...
    def process_statement(self, statement):
        ...
        if statement_expression.elem_type in ["+", "-"]:
            self.variable_to_value[statement_name] = self.evaluate_expression_assignment(statement_expression)
    ...
    def evaluate_expression_operation(self, expression):
        ...
        if op1_type == "var":
            ...
            first_value = expression.get("op1").dict
        ...
        if op2_type == "var":
            ...
            second_value = expression.get("op1").dict
        elif op2_type == "string":
            second_value = expression.get("op1").dict
        elif op2_type == "expression_operation":
            second_value = evaluate_expression_operation(expression.get("op1"))
        elif op2_type == "expression_fcall":
            second_value = evaluate_expression_function_call(expression.get("op1"))
        else:
            self.error(ErrorType.NAME_ERROR, "Operation value does not exist")
